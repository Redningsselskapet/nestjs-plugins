...
- **filterSubject**: string - When consuming from a stream with a wildcard subject, the Filter Subject allows you to select a subset of the full wildcard subject to receive messages from. NOTE: it cannot be used with `filterSubjects`.
- **filterSubjects**: string[] - When consuming from a stream with a wildcard subject, the Filter subjects allow you to select a subset of the fill wildcard subject to receive message from. NOTE: it cannot be used with `filterSubject`.
